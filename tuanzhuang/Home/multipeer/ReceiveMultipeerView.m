...
@interface ReceiveMultipeerView ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation ReceiveMultipeerView
...
-(void)dealloc{
    NSLog(@"ReceiveMultipeerView dealloc!");
}
...
- (void)layoutTop{
    ...
    if(code!=nil && ![code isEqualToString:@""]){
        title.text = [NSString stringWithFormat:@"%@  -  %@",@"数据接收信息",code];
    }else{
        title.text = @"数据接收信息";
    }
    ...
    [cancelBtn setTitle:@"收起" forState:UIControlStateNormal];
